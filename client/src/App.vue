<template>
  <v-app style="background: rgb(46, 46, 46)">
    ...
    >
      ...
      <span v-on:click="download" style="cursor: pointer" class="hidden-sm-and-down toolbar">
      ...
      <v-menu open-on-hover down offset-y>
        <template v-slot:activator="{ on }">
          <span v-on="on" style="cursor: pointer" class="hidden-sm-and-down toolbar">
        ...
        <v-list dark>
          <v-list-tile v-for="item in aboutList" :key="item" :to="'about' + item">
            <v-list-tile-title>
      ...
      <span
        v-if="isLoggedIn"
        v-on:click="loggedOut"
        style="cursor: pointer"
        class="hidden-sm-and-down toolbar"
      >
      ...
      <v-toolbar-items>
        <v-btn
          ...
          color="white"
        >
          <v-icon color="white">message</v-icon>message
        </v-btn>
      </v-toolbar-items>
    </v-toolbar>

    <v-navigation-drawer class="menuIndex" v-model="drawer" absolute temporary dark>
      <v-list class="pt-0">
        <v-list-tile to="/download">
          <v-list-tile-content>
            <v-list-tile-title>
              <v-icon color="rgb(240, 226, 150)">cloud_download</v-icon>Download
            </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
        ...
        <v-list-tile v-if="isLoggedIn" @click="loggedOut">
          <v-list-tile-content>
            <v-list-tile-title>
              <v-icon color="rgb(240, 226, 150)">exit_to_app</v-icon>Logout
            </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
        <v-list-tile href="https://discord.gg/aPg5W2k" target="_noblank">
          <v-list-tile-content>
            <v-list-tile-title>
              <v-icon color="rgb(240, 226, 150)">message</v-icon>Message
            </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
      </v-list>
    ...
    <v-footer fixed color="black" class="toolbarIndex footerBorder pa-3 mt-2">
...
export default {
  ...
  methods: {
    ...
    logged() {
      axios.get(this.urlGet).then(res => {
